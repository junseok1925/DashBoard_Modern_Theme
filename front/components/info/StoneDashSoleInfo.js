...
function DashSoleInfo({ theme, zone, scanners, zonedatas }) {
  ...
  // 권역별 존정보 넣기
  areaZones.set("경주돌담길 전체", ["벚꽃놀이터", "FnB", "프리마켓", "친환경 쉼터", "라이트쇼"]);
  ...
  const getAPIdata1 = async () => {
    // 오늘 누적방문객
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_pohang_URL}/DeviceCountHourly?unit=1d-1h`); //오늘누적
      for (let i of response.data) {
        for (let j of zoneData) {
          if (i.zone_id == j.zone && i.data > j.todayVisitTemp) {
            j.todayVisit = i.data;
      ...
      setZoneInfo11(zoneInfoAll.get("FnB"));
  ...
  const getAPIdata2 = async () => {
    try {
      ...
      const response2 = await axios.get(`${process.env.NEXT_PUBLIC_API_pohang_URL}/DeviceCountHourly`);
      ...
      for (let i of response2.data) {
        for (let j of zoneData) {
          if (i.zone_id == j.zone && i.time > j.revisitTime) {
            j.revisit = i.data;
      ...
      setZoneInfoAll(zoneVisitMap);
      setZoneInfo11(zoneInfoAll.get("33"));
  ...
  const getAPIdata3 = async () => {
    try {
      //const response3 = await axios.get(`${process.env.NEXT_PUBLIC_API_pohang_URL}/DeviceResidenceTime`); //평균체류인원
      const response3 = await axios.get(`${process.env.NEXT_PUBLIC_API_pohang_URL}/DeviceCountHourly?unit=10m`); //평균체류인원
      var tempTime = "2021-04-24T10:50:00.000Z";

      for (let i of response3.data) {
        for (let j of zoneData) {
          if (i.zone_id == j.zone && i.time > j.stayTime) {
      ...
      <div className={"fpa_box " + back}>
        <div className="fpa_box_table">
          <div className="tr">
            ...
            <div className="td">
              <span className="fpa_title">{zoneInfos ? makeNumber(Number(zoneInfos[2])) + "명" : "-"}</span>
            </div>
            <div className="td">
  ...
  return (
    <Block>
      <div className="column">
        ...
        <div className="tr">현재방문</div>
        <div className="tr">오늘방문</div>
        <div className="tr">어제방문</div>
      </div>
