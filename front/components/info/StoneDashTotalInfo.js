...
function DashTotalInfo({ theme, zone }) {
  ...
  const getAPIdata2 = async () => {
    ...
    try {
      ...
      for (let i of response.data) {
        if (i.zone_id === zone) {
          if (i.data != 0 && i.time > timeTemp) {
            ...
            sessionStorage.setItem("전체", i.data);
      ...
      if (cnt == 0 && sessionStorage.getItem("전체")) {
        setMainStackVisitTotal(sessionStorage.getItem("전체"));
      }
    } catch (err) {
      console.error(err);
      setTimeout(() => {
  ...
  const getAPIdata3 = async () => {
    // 이달의 방문객
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_pohang_URL}/DeviceCountDay`);
  ...
  return (
    <StatusBlock className="container">
      <div className={zone === "경주 전체" ? "fpa_box all" : "fpa_box section"}>
        <div className="fpa_box_table">
          <div className="tr">
            ...
            <div className="td">
              <span className="fpa_title">어제 방문객</span>
              <br />
